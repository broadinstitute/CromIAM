...
object Version {
  ...
  def cromwellVersionWithGit: Seq[Setting[_]] =
    Seq(
      ...
      git.baseVersion in ThisBuild := cromiamVersion,
      version in ThisBuild :=
        makeVersion(
          versionProperty = git.versionProperty.value,
          baseVersion = git.baseVersion.?.value,
          headCommit = git.gitHeadCommit.value),
      shellPrompt in ThisBuild := { state => "%s| %s> ".format(GitCommand.prompt.apply(state), cromiamVersion) }
    )

  val writeVersionConf: Def.Initialize[Task[Seq[File]]] = Def.task {
    val file = (resourceManaged in Compile).value / "cromiam-version.conf"
    val contents =
      s"""|version {
          |  cromiam: "${version.value}"
          |}
          |""".stripMargin
